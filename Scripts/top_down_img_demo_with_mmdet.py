...
def main():
    ...
    keypoints_json = []
    person_id = 1  # Initialize person ID for each detected person
    for person in pose_results:
        ...
        person_entry = {
            "person_id": person_id,  # Unique ID for each detected person
            "keypoints": keypoints,
            "score": float(person.get('score', 0)),  # Convert score to float for JSON serialization
        }
        keypoints_json.append(person_entry)
        person_id += 1
    
    # Determine JSON file name based on image name
    json_file_name = os.path.splitext(args.img)[0] + '_keypoints.json'
    output_json_path = os.path.join(args.out_img_root, json_file_name)

    # Save keypoints JSON for the current image
